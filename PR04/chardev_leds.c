...
/*
 * Called when a process writes to dev file: echo XXX > /dev/leds
 */
static ssize_t
device_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
     ...
     */
    char buff2[len];
    if (copy_from_user(buff2, buff, len) != 0) {
    	printk(KERN_ALERT "Error on copy\n");
    	return -EFAULT;
    }
    ...
    for (i = 0; i < len; i++) {
    	aux = buff2[i];
    	if (aux == '1') num_lock = 1;
    	else if (aux == '2') caps_lock = 1;
    	else if (aux == '3') scroll_lock = 1;
    }

    /*
     * Create the leds mask
     */
    int mask = 0x0;
    if (num_lock == 1) mask = (mask | 0x2);
    if (caps_lock == 1) mask = (mask | 0x4);
    if (scroll_lock == 1) mask = (mask | 0x1);

    /*
     * Get driver handler and set led state
     */
